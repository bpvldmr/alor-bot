...
# 2025-07-22  patch-23
#
# • Избирательный cooldown для сигналов **RSI<30 / RSI>70**:
#       ─ CNY 9.26  →  5 часов  (18 000 сек)
#       ─ NG-8.25  →  90 минут  (5 400 сек)
#       ─ все остальные тикеры, как прежде → 60 минут (3 600 сек)
#
#   Реализовано через словарь RSI_CD_MAP и helper-функцию
#   get_rsi_cooldown(sym). В остальной логике ничего не изменено.
# ─────────────────────────────────────────────────────────────────────────────
import asyncio, time
...
last_signal_ts:dict[str, float]  = {}    # timestamp всех прочих сигналов

# ───────── базовые cooldown-ы (по умолчанию) ────────────────────────────────
RSI_COOLDOWN_SEC_DEFAULT = 60 * 60        # 60 минут
GEN_COOLDOWN_SEC         = 60 * 60
TP_COOLDOWN_SEC          = {v["trade"]: 30 * 60 for v in TICKER_MAP.values()}

# ───────── индивидуальные RSI-cooldown-ы (только для 30/70) ────────────────
RSI_CD_MAP = {
    "CNY9.26": 5 * 60 * 60,   # 5 часов
    "NG-8.25": 90 * 60        # 90 минут
}

def get_rsi_cooldown(sym: str) -> int:
    """Возвращает индивидуальный cooldown для RSI<30 / RSI>70."""
    return RSI_CD_MAP.get(sym, RSI_COOLDOWN_SEC_DEFAULT)
...
def update_and_check_cooldown(sym: str, sig: str, now: float, cd: int) -> bool:
    """True — cooldown активен (сигнал игнорируем)."""
    key  = f"{sym}:{sig}"
...
async def process_signal(tv_tkr: str, sig: str):
    ...
    if sig_upper in ("RSI<30", "RSI>70"):
        rsi_cd = get_rsi_cooldown(sym)
        if update_and_check_cooldown(sym, sig_upper, now, rsi_cd):
            return {"status": "rsi_cooldown"}

        want_dir = desired_direction(sig_upper)          # +1 / -1
        same_dir = (pos * want_dir) > 0

        if same_dir:
            side = "buy" if want_dir > 0 else "sell"
            qty  = ADD_QTY[sym]                          # усреднение
        else:
            side = "buy" if want_dir > 0 else "sell"
            qty  = abs(pos) + START_QTY[sym] if pos != 0 else START_QTY[sym]
